...
anything (the Safe-Deletion Playbook in `references/techniques.md` §6 governs deletion).
...
— verify each per `references/techniques.md` §7 before removal. For non-Python repos, apply the §7 method by
...
| Classify files | LLM read-pass over module docstrings + directory names (`references/techniques.md` §1) |
